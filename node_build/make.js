...
Builder.configure({
    ...
    var lto = (GCC !== 'cl') ? '-flto' : '/GL';
    var ltoTest = (GCC !== 'cl') ? ['-x', 'c'] : [];

    CanCompile.check(builder,
        'int main() { return 0; }', [builder.config.cflags, builder.config.flag.lto, builder.config.flag.languageC],
        function(err, can) {
            if (can) {
    ...
    if (builder.config.systemName === 'win32') {
        if (builder.config.gcc !== 'cl') {
            ...
            libuvLib = dependencyDir + '/libuv/out/Release/libuv' + builder.config.ext.lib;
        }
    }
