...
var mkBuilder = function(state) {
    var builder = {
        ...
        buildExecutable: function(cFile, outputFile) {
            if (!outputFile) {
                outputFile = cFile.replace(/^.*\/([^\/\.]*).*$/, function(a, b) {
                    return b;
                });
            }
        ...
        runTest: function(outFile, testRunner) {
            builder.tests.push(function(cb) {
                testRunner(outFile, cb);
            });
        },
...
var probeCompiler = function(state, callback) {
    nThen(function(waitFor) {
        ...
        if (state.gcc === 'cl') {
            compilerType.isMSVC = true;
        }
        else {
            compiler(state.gcc, ['-v'], waitFor(function (ret, out, err) {
                if (ret !== 0) {
                    throw new Error("Failed to probe compiler ret[" + ret + "]\n" + err);
                }
